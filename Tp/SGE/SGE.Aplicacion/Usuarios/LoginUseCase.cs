...
public class LoginUseCase(IUsuarioRepository repo, ITokenProvider tokenProvider, IHashService hashService)

{
    public LoginResponseDTO Ejecutar(LoginDTO req)
    {
        Usuario? user = repo.ObtenerPorEmail(req.Email);
        if (user == null || !hashService.Verify(req.Contrasena, user.ContrasenaHash))
        {
